...
class GoldDocument:
    ...
    def __init__(self, doc_id:str, text:str, annotations:Dict[str,List]):
        ...
        for annotator, ann_by_person in annotations.items():
            ...
            #load all entities from each annotator
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dump_masks", type=str, default=None,
                        help="extracts masks from gold reference and saves in <supplied path>.json") 
    ...
    if args.dump_masks:
        docToSpans = {}
        for docID in gold_corpus.documents:
            spans = []
            for entityID in gold_corpus.documents[docID].entities:
                for span in gold_corpus.documents[docID].entities[entityID].mentions:
                    spans.append(span)

            #new_entity = AnnotatedEntity(entity_id, [(start, end)], need_masking, is_direct, 
            #                                 mention["entity_type"], [need_masking])
            docToSpans[docID] = spans
        with open(args.dump_masks, 'w') as fp:
            json.dump(docToSpans, fp)
    else:
